...
- 遇到无读取权限的源文件或目录时自动列出并跳过，重试当前目录后继续后续任务；权限恢复后再次运行会自动补齐
...
如果源服务器拒绝读取文件或整个目录，app 会把完整路径保存到目标目录的 `.cygentig-transfer/skipped-unreadable-files.txt`，把目录另外保存到 `skipped-unreadable-directories.txt`，并在界面显示“跳过项目”按钮。所有历史记录追加到 `skipped-unreadable-history.log` 和主 `transfer.log`。包含跳过项目的源目录不会被标记为完全完成，确保权限修复后能够继续补齐。
