...
- 遇到无读取权限的源文件时自动列出并跳过，重试当前目录后继续后续任务；权限恢复后再次运行会自动补齐
...
如果源服务器拒绝读取个别文件，app 会把完整路径保存到目标目录的 `.cygentig-transfer/skipped-unreadable-files.txt`，并在界面显示“跳过文件”按钮。包含跳过项目的源目录不会被标记为完全完成，确保权限修复后能够继续补齐。
